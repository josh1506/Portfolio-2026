...
function NavbarDropdown({ activeUser }: { activeUser: ProfileUsersProps }) {
  return (
    <div className="group cursor-pointer flex items-center gap-2">
        ...
        <div className="absolute default-bg top-16 right-14 mt-2 w-56 shadow-lg duration-300 invisible opacity-0 group-hover:visible group-hover:opacity-100 group-hover:delay-0 delay-200 bg-black/5 bg-opacity-80">
            <NavbarUsersList />
        </div>
    </div>
  )
